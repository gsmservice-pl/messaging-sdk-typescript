...
import { messagesSmsGetPrice } from "../../funcs/messagesSmsGetPrice.js";
import * as operations from "../../models/operations/index.js";
import { formatResult, ToolDefinition } from "../tools.js";

const args = {
  request: operations.GetSmsPriceRequestBody$inboundSchema,
};

export const tool$messagesSmsGetPrice: ToolDefinition<typeof args> = {
  name: "messages-sms-get-price",
  description: `Check the price of SMS Messages
...
\`GetSmsPriceResponse\` object will include also \`headers\` array with \`X-Success-Count\` (a count of messages which were processed successfully) and \`X-Error-Count\` (count of messages which were rejected) elements.`,
  ...
  tool: async (client, args, ctx) => {
    const [result, apiCall] = await messagesSmsGetPrice(
      client,
      args.request,
